...
computes and return the loss w.r.t. data `X̃`. Optionally nuisance artifacts can be provided.
"""
...
function train_!(
    ...
    for e = 1:E
        ...
        if e % ssi == 0
            ...
            X_gen = @allowscalar @views generate(d, tfrec.model, tfrec.O, d.X[1, :], vcat(d.R, d.R_test), T_total)
            ...
            pe = prediction_error(d, tfrec.model, tfrec.O, d.X, d.R, PE_n)
            pe_test = isempty(d.X_test) ? NaN : prediction_error(d, tfrec.model, tfrec.O, d.X_test, d.R_test, PE_n)
            # progress printing
