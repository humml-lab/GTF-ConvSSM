...
function train_!(
    ...
    for e = 1:E
        ...
        for sₑ = 1:Sₑ
            # sample a batch
            X̃, R̃, X̃_deconv, R̃_deconv = sample_batch(d, T̃, S)
            ...
            grads = Flux.gradient(θ) do
                Lₜᵣ = loss(d, tfrec, X̃, R̃, X̃_deconv, R̃_deconv, d.hrf, conv_mat)
        ...
        if e % ssi == 0
            ...
            # generated trajectory (R does not affect the dynamics and therefore can used also at other positions)
            # only the time of the first R_deconv must match X_deconv to be able to get the latent state z right
            #(inverting the observation model)
            X_gen = @allowscalar @views generate(d, tfrec.model, tfrec.O, d.X_deconv[1+cutl, :], d.R_deconv[1+cutl, :], vcat(d.R, d.R_test), T_total + length(d.hrf) - 1)
